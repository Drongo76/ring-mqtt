...
export class H264RtpFrameGate {
    ...
    push(packet) {
        ...
        if (this.frames.has(frame.key)) this.ensureReorderTimer(frame)
    ...
    ensureReorderTimer(frame) {
        if (frame.timer || !this.frames.has(frame.key)) return
        const isCurrentSyncedFrame = this.synced && this.expectedSequence !== null &&
            frame.ssrc === this.mediaSsrc && frame.packets.has(this.expectedSequence)
        if (!frame.markerSeen && !isCurrentSyncedFrame) return
    ...
    getTimeoutRejectReason(frame) {
        if (!frame.markerSeen) return 'reorder-timeout-missing-marker'
        const startSequence = this.synced && this.expectedSequence !== null
            ? this.expectedSequence
            : this.findKeyframeStart(frame)
