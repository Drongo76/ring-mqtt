...
export class AdaptiveFrameSelector {
    constructor({
        ...
        // Distance calculation is expensive (160x90 luma + brightness compensation).
        // A directed candidate pair always has the same immutable luma buffers, so
        // reuse the exact result instead of recomputing F1-F2/F1-F3 for every triple.
    ...
    findBestPair(requireSeparation) {
        ...
        for (let i = 1; i < this.candidates.length - 1; i++) {
            ...
            for (let j = i + 1; j < this.candidates.length; j++) {
                ...
                const d12 = this.getPairDifference(first, second)
                const d23 = this.getPairDifference(second, third)
                const d13 = this.getPairDifference(first, third)
