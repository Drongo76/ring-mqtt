...
export const DEFAULT_KI_BURST_HARD_SAFETY_TIMEOUT_MS = 25000
...
export function getObservationStopDelayMs(observationWindowMs) {
    return Math.max(1, observationWindowMs)
}
...
const CANDIDATE_JPEG_FILENAME = /^candidate-(\d{6})\.jpg$/

export function inspectCandidateJpegFiles(entries, metadataSourceIndices = []) {
    const physical = []
    for (const entry of entries || []) {
        const filename = typeof entry === 'string' ? entry : entry?.name
        const match = filename?.match(CANDIDATE_JPEG_FILENAME)
        if (!match) continue
        const sourceIndex = Number(match[1])
        if (!Number.isInteger(sourceIndex)) continue
        physical.push({ filename, sourceIndex })
    }
    physical.sort((a, b) => a.sourceIndex - b.sourceIndex || a.filename.localeCompare(b.filename))

    const existingSourceIndices = [...new Set(physical.map(entry => entry.sourceIndex))]
    const existing = new Set(existingSourceIndices)
    const normalizedMetadataSourceIndices = [...new Set(
        (metadataSourceIndices || []).filter(index => Number.isInteger(index) && index >= 0)
    )].sort((a, b) => a - b)
    const missingMetadataSourceIndices = normalizedMetadataSourceIndices.filter(index => !existing.has(index))

    return {
        files: physical.map(entry => entry.filename),
        jpegCount: physical.length,
        minSourceIndex: existingSourceIndices.length ? existingSourceIndices[0] : null,
        maxSourceIndex: existingSourceIndices.length ? existingSourceIndices.at(-1) : null,
        candidate000086Present: existing.has(86),
        existingSourceIndices,
        metadataSourceIndices: normalizedMetadataSourceIndices,
        missingMetadataSourceIndices
    }
}

export function restrictSelectorCandidatesToExistingJpegs(selector, existingSourceIndices) {
    if (!selector || !Array.isArray(selector.candidates)) throw new TypeError('KI Burst selector candidates are required')
    if (selector.finalized) throw new Error('KI Burst selector cannot be restricted after finalization')

    const existing = new Set(existingSourceIndices || [])
    const removedSourceIndices = selector.candidates
        .filter(candidate => !existing.has(candidate.sourceIndex))
        .map(candidate => candidate.sourceIndex)
    selector.candidates = selector.candidates.filter(candidate => existing.has(candidate.sourceIndex))
    selector.firstCleanFrameAt = selector.candidates[0]?.observedAt || null

    return {
        removedSourceIndices,
        eligibleSourceIndices: selector.candidates.map(candidate => candidate.sourceIndex)
    }
}

export async function loadSelectedCandidateJpegs(tempDir, selected) {
    const frames = []
    const sourceIndices = []
    for (const source of selected || []) {
        const sourceIndex = source?.sourceIndex
        const frame = await readFile(join(tempDir, candidateFilenameForSourceIndex(sourceIndex)))
        if (!frame.length) throw new Error(`KI Burst selected JPEG is empty for source index ${sourceIndex}`)
        frames.push(frame)
...
export class Build12StreamingSession extends StreamingSession {
    async captureJpegBurst({
        ...
        emitOnce('burst_started', { at: new Date(burstStartedAtMs).toISOString(), hardSafetyTimeoutMs })
        let answerTimeout
        const answerRemainingMs = Math.max(1, hardDeadline - Date.now())
        emitOnce('waiting_for_sdp_answer', { at: new Date().toISOString(), answerRemainingMs })
        const ringSdp = await Promise.race([
            firstValueFrom(this.connection.onCallAnswered),
            firstValueFrom(this.onCallEnded),
            new Promise((_, reject) => {
                answerTimeout = setTimeout(() => {
                    emitOnce('sdp_answer_timeout', { at: new Date().toISOString(), hardSafetyTimeoutMs })
        ...
        emitOnce('sdp_answer_received', { at: new Date().toISOString(), sdpLength: ringSdp.length })
        ...
        let candidateJpegInventory = null
        let excludedMissingJpegSourceIndices = []
        ...
        const captureDiagnostics = () => ({
            ...
            candidateEvaluations: candidateEvaluations.map(entry => ({ ...entry })),
            ...
            candidateJpegInventory,
            excludedMissingJpegSourceIndices: [...excludedMissingJpegSourceIndices],
        ...
        const startObservationWindow = () => {
            ...
            if (hardSafetyTimeout) {
                clearTimeout(hardSafetyTimeout)
                hardSafetyTimeout = null
            }
            emitOnce('observation_window_started', { at: new Date(firstCleanWallClockMs).toISOString(), observationWindowMs })
            const observationDelayMs = getObservationStopDelayMs(observationWindowMs)
            observationTimeout = setTimeout(() => {
                emitBurstDiagnostic(burstId, 'observation_complete', {
                    at: new Date().toISOString(),
                    observationWindowMs,
                    actualObservationMs: Date.now() - firstCleanWallClockMs
                })
        ...
        try {
            ...
            candidateJpegInventory = inspectCandidateJpegFiles(
                await readdir(tempDir),
                selector.candidates.map(candidate => candidate.sourceIndex)
            )
            const existingJpegs = new Set(candidateJpegInventory.existingSourceIndices)
            for (const evaluation of candidateEvaluations) {
                evaluation.jpegPresent = existingJpegs.has(evaluation.index)
            }
            emitBurstDiagnostic(burstId, 'candidate_jpeg_inventory', {
                files: candidateJpegInventory.files,
                jpegCount: candidateJpegInventory.jpegCount,
                minSourceIndex: candidateJpegInventory.minSourceIndex,
                maxSourceIndex: candidateJpegInventory.maxSourceIndex,
                candidate000086Present: candidateJpegInventory.candidate000086Present,
                metadataSourceIndices: candidateJpegInventory.metadataSourceIndices,
                missingMetadataSourceIndices: candidateJpegInventory.missingMetadataSourceIndices
            })

            const restriction = restrictSelectorCandidatesToExistingJpegs(selector, candidateJpegInventory.existingSourceIndices)
            excludedMissingJpegSourceIndices = restriction.removedSourceIndices
            emitBurstDiagnostic(burstId, 'candidate_jpeg_eligibility', {
                eligibleCandidateCount: restriction.eligibleSourceIndices.length,
                eligibleSourceIndices: restriction.eligibleSourceIndices,
                excludedMissingJpegSourceIndices
            })
            ...
            selectedSourceIndices = selected.map(frame => frame.sourceIndex)
            emitBurstDiagnostic(burstId, 'candidate_jpeg_selection', {
                selectedSourceIndices,
                allSelectedJpegsPresent: selectedSourceIndices.every(index => existingJpegs.has(index))
            })
            emitBurstDiagnostic(burstId, 'selection_complete', {
                at: new Date().toISOString(),
                selectedSourceIndices
            })
            ...
            let loaded
            try {
                loaded = await loadSelectedCandidateJpegs(tempDir, selected)
            } catch (error) {
                error.kiBurstDiagnostics = captureDiagnostics()
                emitBurstDiagnostic(burstId, 'candidate_jpeg_read_failed', {
                    message: error.message,
                    selectedSourceIndices,
                    inventory: candidateJpegInventory
                })
                throw error
            }
            const frames = loaded.frames
            const frameHashes = loaded.frameHashes
            const paths = []
            ...
            emitBurstDiagnostic(burstId, 'frames_persisted', {
                at: new Date().toISOString(),
                paths
            })
