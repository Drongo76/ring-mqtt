    ...
    async captureJpegBurst({
        ...
        const evaluateReadyCandidates = () => {
            while (!processingError &&
                ...
                if (decoded.pts !== full.pts || decoded.pts !== lumaDiagnostic.pts ||
                    decoded.ptsTime !== full.ptsTime || decoded.ptsTime !== lumaDiagnostic.ptsTime) {
                    processingError = new Error(`KI Burst decoded/full/luma PTS mismatch at source index ${index}`)
                    requestStop('frame-identity-error')
                    return
                }

                const previous = selector.candidates.at(-1)
                if (previous && decoded.pts <= previous.pts) {
