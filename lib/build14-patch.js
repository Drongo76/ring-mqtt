import { compactRtpIntegrityForHomeAssistant } from './build12-patch.js'
...
export function extractBuild14Diagnostics(details = {}) {
    return {
        targetFrameOffsetsMs: Array.isArray(details.targetFrameOffsetsMs) ? details.targetFrameOffsetsMs : [0, 800, 1600],
        actualFrameOffsetsMs: Array.isArray(details.frameOffsetsMs) ? details.frameOffsetsMs : [],
...
function extractBuild14PublicDiagnostics(details = {}) {
    return {
        targetFrameOffsetsMs: Array.isArray(details.targetFrameOffsetsMs) ? details.targetFrameOffsetsMs : [0, 800, 1600],
...
export function buildBuild14PublicAttributes(camera, diagnostics = {}) {
    const currentAttributes = camera.data?.ki_burst?.attributes || {}
...
export function publishBuild14BurstDiagnostics(camera, details) {
    const entity = camera.entity?.ki_burst_status
    if (!entity?.json_attributes_topic || !camera.data?.ki_burst?.attributes) return

    // Keep the full worker diagnostics available internally for DEBUG/DIAG, but never
    // merge raw frameHashes or raw RTP diagnostic arrays back into public MQTT state.
    const diagnostics = extractBuild14Diagnostics(details)
    camera.data.ki_burst.build14Diagnostics = diagnostics
    const attributes = buildBuild14PublicAttributes(camera, diagnostics)

    camera.data.ki_burst.attributes = attributes
    camera.mqttPublish(entity.json_attributes_topic, JSON.stringify(attributes), 'attr')
}

export function republishStoredBuild14Diagnostics(camera) {
